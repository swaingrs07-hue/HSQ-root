...
- **Resend**: Email delivery service for automated booking confirmation emails. Service module at `server/email-service.ts`, sends branded HTML emails on booking confirmation and first payment receipt. Triggers: (1) `sendBookingConfirmationEmail` — resident email with receipt PDF attached, sent on booking confirm or first payment; (2) `sendParentBookingConfirmationEmail` — parent/guardian email with receipt PDF attached, triggered on first payment (mark-payment-done or online payment). Server-side PDF receipt generation in `server/receipt-pdf.ts` using jsPDF. Email status (sent/failed) logged to CRM activity logs.