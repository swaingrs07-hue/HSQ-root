...
-   **Windows-Chrome Performance Invariants** (added because the hero video plays smoothly on iOS but the homepage + plans-hallway noticeably lagged on Windows-Chrome even on a decent gaming laptop — Apple's compositor handles patterns Windows-Chrome cannot). Touch any of these at your own risk:
    ...
    4.  **Tubes WebGL background** (`client/src/components/tubes-cursor-background.tsx`): MUST run an adaptive baseline-FPS measurement (currently 900 ms sample, ≥**55** fps threshold) BEFORE instantiating the heavy WebGL context. On Windows + integrated/older discrete GPU the iridescent shader stacked with everything else can't sustain 60 fps and the user perceives the whole site as laggy. Falling below the threshold MUST call `onFailure` (the existing graceful-fallback path that disables the tubes for the rest of the session). Skip the gate when `document.visibilityState !== 'visible'` to avoid false positives on hidden tabs / headless test environments where RAF rate is throttled. The 55 fps threshold (raised from 45 in May 2026) reflects that the 900 ms baseline measurement runs BEFORE heavy assets are loaded, so the idle baseline overestimates the sustained framerate by a wide margin — a higher threshold gates out mid-tier devices like the Dell G15 5515 that pass at idle but stutter under realistic scroll/decode load.
    5.  **Hero `<video>` element** (`client/src/pages/home.tsx`): MUST NOT have `mask-image` / `WebkitMaskImage` applied directly while playing on non-Apple compositors. Same Windows-Chrome software-compositor trap as rule #2 above. The mask is gated behind a `IS_FAST_VIDEO_COMPOSITOR` UA check (matches `Mac|iPhone|iPad|iPod`) so Apple users still get the bottom-feather aesthetic that lets the iridescent tubes glow through the lower 30% of the video, while Windows/Linux users get a fully opaque hero video with the existing dark bottom-gradient overlay (the `absolute inset-0 z-[5]` div directly below the video) handling the visual hand-off into the page below. The placeholder `<img>` is a static layer and may keep its mask unconditionally — only PLAYING video triggers the compositor regression.
    6.  **Hero `<video>` element**: MUST be paused via the `heroInViewport` IntersectionObserver state whenever the hero scrolls off-screen. The browser will happily keep decoding 24fps of H.264 forever even when the element isn't visible, which on Windows + integrated GPU is meaningful per-frame work stacked on top of everything else further down the page. Resume on re-entry, but skip play() when `document.visibilityState !== 'visible'` (autoplay handles that case for free when the tab returns).
    7.  **Plans-hallway cards** (`client/src/components/plans-hallway.tsx`): each card MUST set `willChange: 'transform'` + `contain: 'layout paint'` so the compositor promotes them to their own GPU layers. Without this, the parent track's scroll-driven `translateZ` forces Chrome to re-rasterize every card's box-shadow + content on every scroll frame. Card resting box-shadow blur radii MUST stay at ≤40px / ≤25px (was 80px / 60px) — blur radius dominates shadow rendering cost and the original values made cards visibly drag during scroll on Windows-Chrome. The hover state may use larger radii (~50px) since hover triggers a one-shot rasterization. The card's `transition` rule MUST NOT list `transform` — the card transform never animates (the parent track owns the scroll-driven motion), so listing it just makes Chrome instrument every transform change for transition purposes during scroll.
    8.  **Tubes graceful fallback** (`client/src/components/iridescent-fallback-background.tsx`, mounted from `client/src/components/layout.tsx`): when the WebGL tubes are disabled by rule #4's FPS gate (or the device has no WebGL2/WebGPU context at all), the layout MUST mount the lightweight CSS-only `IridescentFallbackBackground` in their place — never leave the user staring at a flat `#050505` page. The fallback is a 5-blob radial-gradient stack using the same palette (`#ff008a`, `#2e6cfe`, `#60aed5`, `#fe8a2e`, `#83f36e`) blended with `mix-blend-mode: screen` and animated via long-period (28–35 s) `transform`-only keyframes — GPU-composited, no shader, no decode, no blur, no layout/paint. The fallback element fades in over 1.4 s on mount so the hand-off from the FPS gate's failure isn't a hard pop. The fallback MUST NOT use `filter: blur()` (expensive on Windows — soft edges come from the gradient stop position alone) and each blob MUST stay ≤75vmax. The fallback is gated behind `!prefersReducedMotion && !saveDataMode` so users who explicitly opted out of motion/data still get a plain dark page. The veil overlay is slightly darker than the WebGL veil (`rgba(5,5,5,0.32)` vs `0.22`) because the CSS gradients are flatter and need a touch more contrast to keep text readable.
