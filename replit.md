...
-   **Google Search Console Change-of-Address (hsquareliving.com → hsquare.in)**: The 301 redirect above is the technical half of the domain move; the other half is telling Google to migrate ranking signals via the GSC **Change of address** tool. Step-by-step procedure (verify both domains as Owner, file change-of-address on the OLD property pointing at the NEW, re-submit sitemap under hsquare.in) and a filing log are in `docs/gsc-change-of-address.md`. The actual GSC submission requires a human with Owner access — fill in the "Filed on" date in that runbook once submitted so future operators know when to expect Google's index to fully transition (~2–4 weeks after filing). Do NOT touch hsquareliving.com MX records or let its registration lapse — the 301 must stay live indefinitely.
