...
-   **Hero Video Encoding Budget**: All hero-slide MP4s must be re-encoded to a web-safe budget before being pointed at by the active `hero_slides.video_url`. Target: H.264 main profile @ 1600×900 (or 1280×720), 24 fps, **CRF 26 with `-maxrate 2500k -bufsize 5000k`** so the overall stream sits at **≤ 2.5 Mbps video / ≤ 3 Mbps overall**, **closed GOP every 2 s** (`-g 48 -keyint_min 48 -sc_threshold 0` so the loop seam re-decodes from a clean keyframe and the browser stops re-firing `canplay` on every loop), AAC LC 96 kbps stereo 48 kHz audio, `-movflags +faststart` (moov before mdat), `-map_metadata -1` (no C2PA / `jumb` / `uuid` atoms — the Task #144 invariant). The reusable runbook is `scripts/optimize-hero-video.ts` (downloads source, re-encodes, verifies atom layout + no-C2PA + budget, uploads as a NEW private object — never mutates the source so rollback is one SQL UPDATE). Task #144 produced a clean-but-heavy MP4 (8.24 MB / 8.5 Mbps / 1920×1080) which made the loop seam visibly stutter (`[hero-video] canplay -> play()` re-firing every 7–8 s in the live console — the browser reporting it had to re-buffer to keep the loop alive). Task #146 ran the optimiser against that object and produced `/objects/uploads/c7533a3f-f2a1-492d-bcbb-23d9b0e10428` (2.57 MB / 2.69 Mbps / 1600×900 / 24 fps / closed GOP every 48 frames / faststart held / no C2PA), repointed the active `hero_slides` row at it, and the `canplay` re-fire loop disappeared. The original heavy object `/objects/uploads/00cd147c-9161-42e4-a63d-c97be0c0a7e7` is left intact in storage for audit / rollback.
