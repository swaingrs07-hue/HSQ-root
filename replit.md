...
-   **Hotels Module (`/hotels/*`)**: A separate luxury Valora-style guest portal that lives alongside the hostel app and reuses the existing `properties` table (filtered by `category="hotel"`), `roomTypes`, `bookings`, and `payments` — no duplicate entities. Includes:
    ...
    - **Theming**: dark-by-default with a sun/moon toggle pill in the Hotels nav (desktop + mobile) that flips the chrome to a warm ivory palette and persists in `localStorage["hotels-theme"]`. Implemented as CSS overrides scoped under a `.hotels-light` class on the HotelsLayout root in `client/src/index.css` (re-skins `text-white/{N}`, `border-white/{N}`, `bg-white/5`, `bg-black`, `bg-[#0a0a0a]`, `bg-black/{30,40,70,95}`, `glass-nav-h`). Inline section/glass backgrounds use `var(--hotels-*, <dark-fallback>)` so the variables flip with the class. Hero overlays painted over images stay moody intentionally so over-image white text remains legible in both themes. Gold (#c5a059) brand accent is preserved everywhere.
