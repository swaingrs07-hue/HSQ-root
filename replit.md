...
- **Virtual Property Tour & Bed Booking**: Dedicated property pages with virtual tours, interactive floor/room/bed selection, room typology, and admin tools for visual management. Includes plan assignment (plans can be linked to multiple room types via `linkedRoomTypeIds` array) and tier-specific bed highlighting.
...
- **HMS Property Sync**: Functionality to link/unlink properties to an external Hostel Management System, including verification and auto-matching. Includes **CRM → HMS Season Sync** that pushes active bookings to HMS by matching residents via phone/email, with sync status tracking (synced/partial/failed) on season cards.
