...
- **SEO Foundations**: Title tags, meta description, canonical URLs (dynamic per route via JS), robots.txt, dynamic sitemap.xml (filters to `status='published'` properties only), JSON-LD structured data (Organization + multiple Hostel entries per location area with amenityFeature and containedInPlace for nearby colleges), Open Graph/Twitter Card tags, dynamic page titles per route in App.tsx. Comprehensive keyword meta tag covering primary (Hsquare Hostel, co-living Mumbai), secondary (hostel near NMIMS/Mithibai/Mukesh Patel), and long-tail keywords. Homepage includes a "Near Top Colleges" section (6 college cards with distances) and an SEO content block with keyword-rich paragraphs for Google crawlability.
...
- **Season/Batch CRM Module**: Management of academic seasons/batches as CRM source-of-truth, including CRUD operations, resident status tracking, and an "End Season" flow for booking processing. Resident sync determines RETAINED vs New Booking status using `customerType` column on bookings: `"student"` (created from HMS registered resident) → RETAINED, `"walk_in"` or `"lead"` → New Booking. For legacy bookings without `customerType`, falls back to HMS phone matching.
...
- **Hero Video Slides**: Hero carousel supports both image and video backgrounds. Admin can upload mp4/webm videos via the Hero Slides admin page. Video slides play full-screen (muted, autoplay, loop, playsInline) with the image as poster/fallback. Video slides auto-advance after 15 seconds, image slides after 6 seconds. Schema: `hero_slides.videoUrl` (nullable text). Title/subtitle/caption animations use cinematic blur-to-sharp reveals with enhanced text shadows.
...
- **Wallet Credit Auto-Renewal**: Background job (every 6 hours) automatically renews alacarte credits monthly for active booking packages. Calculates months since package start date, checks how many renewals have been applied (via `refType: "package_credit_renewal"` in wallet_ledger), and credits any missing months. Skips ended packages. Initial credit uses `refType: "package_credit"`, renewals use `"package_credit_renewal"`.
...
- **Hostel Management System (HMS)**: External system for property syncing. Base URL configured via `HMS_API_URL` env var (default: `https://hsquarehostels.com`). Auth uses `HMS_API_KEY` (`Authorization: Bearer` header) with fallback to legacy `HOSTEL_FLOW_EMAIL`/`HOSTEL_FLOW_PASSWORD` JWT login. Centralized via `getHMSAuthHeaders()` in `server/routes.ts`. Auto-sync uses dedicated `syncBookingToHMS()` helper in `server/hms-sync.ts` which calls `/sync/create-resident` endpoint for upsert-based resident sync. **Image URL sync**: Both `autoSyncBookingToHMS()` and `/sync/first-payment` include `idProofUrl`, `photoUrl`, and `documentUrls` in sync payload. Relative object storage paths are resolved to full public URLs via `resolvePublicUrl()` in `server/hms-sync.ts` using `APP_PUBLIC_URL` (default: `https://hsquare.in`). **Bidirectional sync**: HMS can push resident edits back to CRM via `PUT /api/hms/residents/update` (matches by phone/email, updates residentDetails + booking fields, logs to audit trail). **Wallet sync endpoints** (auth: `hmsApiKeyAuth`): `POST /sync/wallet-debit` (debit wallet for à la carte orders, checks balance, refType=`alacarte_order`), `POST /sync/wallet-credit` (credit wallet for refunds/cancels, refType=`order_refund`), `GET /sync/wallet-balance?bookingCode=X` (check balance + recent transactions). All require `bookingCode` or `phone` for booking lookup. **HMS API endpoints** (auth: `hmsApiKeyAuth` via `Authorization: Bearer <HMS_API_KEY>`): `GET /api/hms/bookings` (list), `GET /api/hms/bookings/:identifier` (single by code/phone), `GET /api/hms/bookings/:identifier/receipt` (HTML or `?format=json`), `PUT /api/hms/residents/update`, `POST /sync/first-payment` (webhook from CRM on first payment — creates/syncs resident in HMS, sends welcome email to resident). All booking endpoints include full package details with `features` array (from `package_items` table: type, label, value, includedQty, unit, isOptional) alongside package metadata (tagline, occupancy, locationInfo). Each booking also returns `hmsStatus` (HMS-friendly display label) derived from CRM status + season retention: "Retained" (season RETAINED), "Not Retained" (season NOT_RETAINED), "New Booking" (confirmed), "Active", "Completed", etc.
- **Resend**: Email delivery service for automated booking confirmation emails. Service module at `server/email-service.ts`, sends branded HTML emails with receipt PDF attached. Only **parent/guardian** emails are sent (`sendParentBookingConfirmationEmail`) — resident emails are disabled. Triggers: on every payment (mark-payment-done or online payment). Admins can also manually resend via "Send Parent Email" button. Server-side PDF receipt generation in `server/receipt-pdf.ts` using jsPDF. Email status (sent/failed) logged to CRM activity logs.