...
${allPages.map(p => `  <url>
  ...
  app.patch("/api/admin/properties/:id", authMiddleware, roleMiddleware("admin"), async (req: AuthRequest, res) => {
    try {
      ...
      const allowedFields = ["name", "displayName", "category", "bookingMode", "location", "address", "city", "phone", "email", "amenities", "rules", "mapsUrl", "imageUrl", "highlights", "status", "virtualTourUrl", "virtualTourProvider", "propertyCode", "tourOverviewImages", "includedServices", "moveInCharges", "mapLatitude", "mapLongitude"];
      const updates: any = {};
        ...
        if (roomTypesList.length > 0 && hasBedsToGenerate) {
          ...
          for (const rt of roomTypesList) {
            ...
            for (let r = 0; r < roomsNeeded; r++) {
              ...
              const bedsForRoom: Parameters<typeof storage.createBeds>[0] = [];
              ...
              if (bedsForRoom.length > 0) {
                await storage.createBeds(bedsForRoom);
              }
              roomCounter++;
            }
          }
          ...
          await db.update(schema.floors).set({ totalBeds: totalGenerated, availableBeds: totalGenerated }).where(eq(schema.floors.id, floor.id));
