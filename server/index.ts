...
(async () => {
  ...
  // bookings so a sales exec's "My Bookings" page credits them for bookings
  // generated by an admin/receptionist on a lead they own/converted. This
  // is idempotent — it only touches rows where assignedSalesExecId is NULL
  // and a leadId is present, and it skips leads with neither an assignee
  // nor a converter (nothing sensible to assign to).
  try {
    const { isNull, isNotNull, inArray } = await import("drizzle-orm");
    const orphanBookings = await db
      .select({ id: schema.bookings.id, leadId: schema.bookings.leadId })
      .from(schema.bookings)
      .where(and(
        isNull(schema.bookings.assignedSalesExecId),
        isNotNull(schema.bookings.leadId),
      ));
    if (orphanBookings.length > 0) {
      const leadIdSet = Array.from(new Set(orphanBookings.map(b => b.leadId!).filter(Boolean)));
      const leadOwners = leadIdSet.length > 0
        ? await db
            .select({
              id: schema.leads.id,
              assignedToId: schema.leads.assignedToId,
              convertedByUserId: schema.leads.convertedByUserId,
            })
            .from(schema.leads)
            .where(inArray(schema.leads.id, leadIdSet))
        : [];
      const ownerByLead = new Map<string, string>();
      for (const lead of leadOwners) {
        const owner = lead.assignedToId || lead.convertedByUserId;
        if (owner) ownerByLead.set(lead.id, owner);
      }
      let backfilled = 0;
      for (const booking of orphanBookings) {
        const owner = ownerByLead.get(booking.leadId!);
        if (!owner) continue;
        await db
          .update(schema.bookings)
          .set({ assignedSalesExecId: owner })
          .where(eq(schema.bookings.id, booking.id));
        backfilled++;
      }
      if (backfilled > 0) {
        console.log(`[Startup] Back-filled assignedSalesExecId on ${backfilled} lead-linked booking(s)`);
      }
    }
  } catch (e) {
    console.error("[Startup] Failed to back-fill assignedSalesExecId on lead-linked bookings:", e);
  }
